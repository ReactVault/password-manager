...
describe('db unit test', () => {
  ...
  const [sitePassword, siteUsername, site] = ['testSitePassword', 'testSiteUsername', 'testSite'];
  ...
  describe('#find/update', () => {
    ...
    test('create userInfo table for the first time', async () => {
      const text = 'CREATE TABLE testname (_id INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY, website VARCHAR NOT NULL UNIQUE, user_name VARCHAR, email VARCHAR NOT NULL, password VARCHAR NOT NULL);'
    ...
    test('update the password and username for existed website', async () => {
      const [updatedPassword, updatedUsername] = ['UpdatedPassword', 'UpdatedName']
      const text = 'UPDATE testname SET password = $1, user_name = $2 WHERE website = $3 RETURNING *'
